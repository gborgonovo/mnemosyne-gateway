...
### 3.1 `app.py` (The Streamlit UI)
**Purpose**: The main user-facing application. It orchestrates the interaction loop between the user, the Perception Module, the Initiative Engine, and the LLM.

- **Functions**:
    - Detects "What if" triggers for Sandbox Reasoning.
    - Visualizes the graph state (Heatmap).
    - Manages the chat history and the Alfred persona.
