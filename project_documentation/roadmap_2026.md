...
## 🟢 Fase 6: Mnemosyne Headless & API Gateway (L'Infrastruttura)
Prima di potenziare il ragionamento, dobbiamo rendere Mnemosyne un'infrastruttura accessibile ovunque come middleware.
- **FastAPI Middleware**: Trasformazione della logica in un servizio API REST "Headless" che agisce da Gateway.
- **Architettura Intercettore**: Mnemosyne si pone come proxy tra i client (es. Open WebUI, mobile app) e l'LLM (Ollama/OpenAI), arricchendo i messaggi con il contesto del grafo in modo trasparente.
- **Agnosticismo dell'Interfaccia**: Sviluppo di un protocollo standard di comunicazione (JSON) che permetta a qualsiasi interfaccia (CLI, IDE, Web) di dialogare con il "cervello" Mnemosyne, mantenendo la personalità di Alfred come layer relazionale unificato.
