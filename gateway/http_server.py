...
try:
    ...
    def handle_enrichment_result(payload):
        # Payload format: {"scope": [...], "payload": {"obs_name": ..., "entities": [...], "relationships": [...]}}
        ...
        if entities and obs_name:
             pm.integrate_entities(entities, obs_name, relationships=relationships, scope=scope)
             logger.info(f"PERCEPTION: Integrated {len(entities)} entities and {len(relationships)} relationships for {obs_name}")
