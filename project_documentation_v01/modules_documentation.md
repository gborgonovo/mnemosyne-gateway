...
- **Key Methods**:
  - `create_fulltext_index()`: [NEW] Initializes the Neo4j Lucene index for search fallback.
  - `search_nodes_fulltext(text, scopes, limit)`: Performs high-speed fuzzy search across all text properties of nodes.
  - `semantic_search(query, llm_provider, scopes, limit)`: [NEW] Orchestrates the 3-layer search strategy (Exact -> Vector -> Full-text).
  - `_extract_primary_type(labels)`: [NEW] Dynamic helper to identify the main type of a node by filtering out scope and administrative labels.
  ...
  - `get_orphan_tasks(scopes)`: [NEW] Finds Task nodes with no relationships and `allow_orphan=false`.
...
- **Features**:
  - `generate_initiatives()`: Generates sidebar suggestions.
  - **Orphan Task Detection**: [NEW] Now queries the graph for isolated tasks to prompt user contextualization.
...
- **Main Methods**:
  ...
  - `check_orphan_tasks()`: [NEW] Flags isolated tasks for the Initiative Engine to process.
  - `apply_temporal_decay()`: Triggers the attention model's decay.
  - **Status**: Now active as a background thread started by the Gateway.
...
- **Security Logic**:
  - Implements an **API Key** authentication layer (`verify_api_key`) via FastAPI Dependencies.
  - Controls access to **Knowledge Scopes** through key-specific authorization maps (`config/api_keys.yaml`).
  - Automatically denies requests if the requested scope is not within the authorized set for the provided key.
...
- **Endpoints**:
  ...
  - `PATCH /nodes/{name}/allow_orphan`: [NEW] Silences future orphan warnings for a specific Task.
